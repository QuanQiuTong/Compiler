...
class Parser:
    ...
    def tmp_variable(self, node):
        ...
        if tmp_id == -2:  
            tmp_id = len(self.tmp_variable_list)  
            self.create_new_tmp_variable_id(node, tmp_id)  

        return "%v" + str(tmp_id) 

    def create_new_tmp_variable_id(self, node, tmp_id):
        node_id = id(node)
        self.tmp_variable_list.append(([node_id], [tmp_id]))  

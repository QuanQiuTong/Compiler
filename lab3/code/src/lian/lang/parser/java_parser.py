    ...
    def class_body(self, node, glang_node):
        ...
        if ("attr" in glang_node and glang_node["attr"] and glang_node["attr"][0] == "record" and glang_node[
            ...
            for parameter in glang_node["parameters"]:
                ...
                # is_name_in_methods = any(parameter_name == method["method_decl"]["name"] for method in glang_node["methods"])
                is_name_in_methods = False
                for method in glang_node["methods"]:
                ...
                if not is_name_in_methods:
                    ...
                    glang_node["methods"].append({"method_decl": {
                        "data_type": parameter["parameter_decl"]["data_type"],
                        "name": parameter_name,
                        "type_parameters": "",
                        "body": [
    ...
    def method_declaration(self, node, statements):
        ...
        if child and child.named_child_count > 0:
            ...
            for p in child.named_children:
                ...
                if len(current_parameter_stmts) > 0:
                    new_parameters.append(current_parameter_stmts[-1])
